...
| `submit` | `List<String>` |  | The list of components whose values must be submitted with the event. Each component defines the data structure for the values it contains. By default, Goowee submits the values of all controls contained in the component. |
...
The `display` method is one of the main features of _Goowee_. It renders the GUI on the server and sends it to the browser.
